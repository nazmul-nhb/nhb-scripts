...
const createExports = (distPath, modules, candidates, patterns, extraStatic) => {
    ...
    for (const { pattern, folderName } of patterns) {
        ...
        for (const match of matches) {
            ...
            const esmPath = normalizeExportPath(path.join(distRoot, 'esm', match.rel.replace(/\.d\.ts$/, '.js')));
            const cjsPath = normalizeExportPath(path.join(distRoot, 'cjs', match.rel.replace(/\.d\.ts$/, '.js')));
