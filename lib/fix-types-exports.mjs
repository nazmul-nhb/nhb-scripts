 ...
 *
 * @typedef {Object} FixTypeExportsOptions
 * @property {string} [distPath] Absolute or relative path to the folder containing dts output
 * @property {string} [packageJsonPath] Absolute or relative path to the package.json
 * @property {string[]} [typeFileCandidates] Candidate type filenames (default: ['types.d.ts', 'interfaces.d.ts'])
 * @property {{pattern:string, folderName:string}[]} [extraPatterns] Extra export scanning patterns, e.g. plugins
 */
...
const normalizePath = (filePath) =>
    path.posix.normalize(filePath.split(path.sep).join(path.posix.sep));
            ...
            if (d.isDirectory()) {
                if (d.name === folderName) {
                    ...
                    for (const file of files) {
                        ...
                        found.push({ name, rel: normalizePath(rel) });
                    }
                } else {
                    traverse(full);
                }
 ...
 */
const createExports = (distPath, modules, candidates, patterns) => {
    /** @type {Exports} */
    const exportsField = {
    ...
    for (const { pattern, folderName } of patterns) {
        ...
        for (const match of matches) {
            const esm = normalizePath(`./dist/esm/${match.rel.replace(/\.d\.ts$/, '.js')}`);
            const cjs = normalizePath(`./dist/cjs/${match.rel.replace(/\.d\.ts$/, '.js')}`);
            const types = normalizePath(`./dist/dts/${match.rel}`);
 ...
 */
const createTypesVersions = (validModules, matchedPatterns) => {
    const versions = {};
    ...
    for (const p of matchedPatterns) {
        versions[`${p.pattern}/${p.name}`] = [`dist/dts/${p.rel}`];
    }
...
export const fixTypeExports = (options) => {
    ...
    pkg.typesVersions = createTypesVersions(validModules, matchedPatterns);
