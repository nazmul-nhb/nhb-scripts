 ...
 * @param {string} base The root distPath to start scanning from
 * @param {string} folderName The folder name to look for (e.g. "plugins")
        ...
        for (const d of fs.readdirSync(dir, { withFileTypes: true })) {
            ...
            if (d.isDirectory()) {
                if (d.name === folderName) {
                    for (const file of fs.readdirSync(full).filter(f => f.endsWith('.d.ts'))) {
...
const createExports = (distPath, modules, candidates, patterns, extraStatic) => {
    ...
    for (const { pattern, folderName } of patterns) {
        ...
        for (const match of matches) {
            const typesPath = normalizeExportPath(path.join(path.dirname(distPath), dtsRoot, match.rel));
            const esmPath = normalizeExportPath(path.join(path.dirname(distPath), 'esm', match.rel.replace(/\.d\.ts$/, '.js')));
            const cjsPath = normalizeExportPath(path.join(path.dirname(distPath), 'cjs', match.rel.replace(/\.d\.ts$/, '.js')));
...
const createTypesVersions = (distPath, validModules, matchedPatterns, extraStatic) => {
    ...
    for (const p of matchedPatterns) {
        const rel = path.join(path.dirname(distPath), dtsRoot, p.rel);
        versions[`${p.pattern}/${p.name}`] = [rel.split(path.sep).join('/')];
    }
