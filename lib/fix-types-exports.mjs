...
const createExports = (distPath, modules, candidates, patterns, extraStatic) => {
    ...
    for (const { pattern, folderName } of patterns) {
        ...
        for (const match of matches) {
            const esm = normalizeExportPath(`./${dist}/esm/${match.rel.replace(/\.d\.ts$/, '.js')}`);
            const cjs = normalizeExportPath(`./${dist}/cjs/${match.rel.replace(/\.d\.ts$/, '.js')}`);
            const types = normalizeExportPath(`./${dist}/dts/${match.rel}`);
