...
async function loadBuiltInTemplate(templateName, moduleName) {
	try {
		...
		throw new Error(
			`Error loading template "${templateName}": ${err.message}`,
		);
	}
}
...
export async function generateModule(moduleName, config) {
	...
	if (
		isNotEmptyObject(config.customTemplates) &&
		config.customTemplates?.[template]
	) {
		const tpl = config.customTemplates[template];
		files = tpl.files;
		destination =
			config.destination || tpl.destination || 'src/app/modules';
	} else {
	...
	try {
		...
		if (!config.force) {
			console.error(
				chalk.red(
					`🛑 Module "${moduleName}" already exists at ${destination}`,
				),
			);
			return;
		}
	...
	console.log(
		chalk.green(
			`✅ Module "${moduleName}" generated successfully in ${destination}`,
		),
	);
}
