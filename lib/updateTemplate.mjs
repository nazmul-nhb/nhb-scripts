...
export function updateCollection(moduleName) {
	...
	const match = content.match(/export const COLLECTIONS = \[(.*?)\] as const;/);
	...
	const matched = match[1]?.trimEnd();

	const useComma = matched?.endsWith(',') ? matched : `${matched},`;

	// Inject the new item before the closing bracket
	const updated = content.replace(
		match[0],
		`export const COLLECTIONS = [${useComma} '${capModule}'] as const;`
	);
